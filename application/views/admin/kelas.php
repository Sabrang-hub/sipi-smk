            ...
            <div class="col-12">
                <div class="card">
                    <div class="card-header">
                        <button title="Tambah" class="btn btn-primary btn-sm float-end" data-bs-toggle="modal" data-bs-target="#modal-form-kelas"><i class="mdi mdi-account-plus"></i> <span class="d-none d-lg-inline">Tambah</span></button>
                        <h5>Daftar <?= @$title ?></h5>
                    </div>
                    <div class="card-body">
...
<div class="modal fade" id="modal-form-kelas" tabindex="-1" aria-labelledby="fm-modal-title" aria-hidden="true">
    <div class="modal-dialog">
        <div class=" modal-content">
            ...
            <form id="fm-kelas" action="<?= base_url('simpan-kelas') ?>" method="POST">
...
<script>
    var data_src = [];
    var modal_form_kelas_id = document.getElementById("modal-form-kelas");
    var modal_form_kelas = new bootstrap.Modal(modal_form_kelas_id, {
        backdrop: 'static',
        keyboard: false,
    });
    $(document).ready(function() {
        ...
        modal_form_kelas_id.addEventListener('hidden.bs.modal', function(event) {
            modal_form_kelas_id.querySelector('.modal-title').textContent = 'Tambah Kelas';
            kelas_id.value = '';
            $('#fm-kelas').trigger("reset");
        });

    });

    function edit_kelas(index, e) {
        var data = data_src[index];
        kelas_id.value = data.id;
        nama.value = data.nama;
        $(".dtr-bs-modal").find('[data-bs-dismiss="modal"]').click();
        modal_form_kelas_id.querySelector('.modal-title').textContent = 'Edit Kelas';
        modal_form_kelas.show();
    }

    function hapus_kelas(id, kelas, e) {
        var $this = $(e.target);
        var src = $this.html();
        ...
        Swal.fire({
            title: 'Anda yakin?',
            html: "kelas (<span class=\"text-success\">" + kelas + "</span>) akan dihapus.",
            icon: 'question',

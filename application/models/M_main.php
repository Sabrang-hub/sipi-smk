...
class M_main extends CI_Model
{
    ...
    public function get_list_data_monitoring_siswa()
    {
        ...
        if ($group_id == '2') {
            ...
            $this->db->where('pembimbing_id', $this->session->userdata('id'));
        }
    ...
    public function rekap_kahadiran($siswa_id)
    {
        ...
        $data = [];
        foreach ($kelompok->result_array() as $data_kel) {
            $date_exist = 0;
            ...
            foreach ($period as $value) {
                ...
                $data_absen = $this->db->where(['siswa_id' => $data_kel['siswa_id'], 'tanggal' => $value->format('Y-m-d')])->get('tbl_absen');
                if ($data_absen->num_rows() > 0) {
                    ...
                    $data_ins = [
                        ...
                        'nama_siswa' => nama('m_siswa', 'nama', ['nis' => $data_kel['siswa_id']]),
    ...
    public function get_list_data_absensi_old()
    {
        $siswa_id = $this->input->post('siswa_id');
        $data_kel = $this->db->where('siswa_id', $siswa_id)->get('tbl_kelompok')->row_array();
    ...
    public function simpan_absensi()
    {
        ...
        if ($group_id == 3) {
            if ($cek->num_rows() > 0) {
                ...
                if ($data['status'] == 1) {
                    if (date('H:i:S') > substr($data['waktu_masuk'], 0, 8) && $data['field'] == 'absen_masuk') {
                        $data_up['status'] = 5;
                    }
                    $data_up[$data['field']] = date('H:i:s');
                }
                if (isset($data['keterangan_siswa'])) {
                    $data_up['keterangan_siswa'] = htmlspecialchars($data['keterangan_siswa'], ENT_QUOTES);
                }
                $sql = $this->db->where(['siswa_id' => $data['siswa_id'], 'tanggal' => $data['tanggal']])->update('tbl_absen', $data_up);
            } else {
                $data_ins = [
                    'siswa_id' => $data['siswa_id'],
                    'nama_siswa' => nama('m_siswa', 'nama', ['nis' => $data['siswa_id']]),
                ...
                if ($data['status'] == 1) {
                    ...
                    $data_ins[$data['field']] = date('H:i:s');
                }
                $sql = $this->db->insert('tbl_absen', $data_ins);
            }
        } else {
            if ($cek->num_rows() > 0) {
                $data_up = [
                ...
                if ($data['status'] == 1) {
                    if ($data['absen_masuk'] > substr($data['waktu_masuk'], 0, 8)) {
                        $data_up['status'] = 5;
                    }
                }
                if (isset($data['keterangan_siswa'])) {
